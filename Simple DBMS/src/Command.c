...
Command_t* new_Command() {
    ...
    cmd->args_len = 0;
    cmd->args_cap = 0;
    cmd->condcount = 0;//condcount initialized
    cmd->condtype = 0;//condtype initialized
    //clear all signals
    return cmd;
}
...
int add_select_field(Command_t *cmd, const char *arg){
    ...
    char **buffer = (char**)malloc(sizeof(char*) * (fields_len+1));
    if(buffer==NULL)    return 0;//check buffer's value
    if(cmd->cmd_args.sel_args.fields){
        memcpy(buffer, cmd->cmd_args.sel_args.fields, sizeof(char*) * fields_len);
        free(cmd->cmd_args.sel_args.fields);
    }
    //create select field
    cmd->cmd_args.sel_args.fields=buffer;
    cmd->cmd_args.sel_args.fields[fields_len] = strdup(arg);
